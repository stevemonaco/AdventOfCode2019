...
namespace Advent.UnitTests
{
    ...
    public class Day7Tests
    {
        ...
        public void GetMaxThrusterOutput_AsExpected(string programString, int expected)
        {
            var program = IntCodeComputer.CreateProgramFromString(programString);
            int[][] phaseOptions = { new[] { 0, 1, 2, 3, 4 } };
            var programs = Enumerable.Repeat(program, phaseOptions.First().Length).ToList();
            var actual = Day7.Program.GetMaxThrusterOutput(phaseOptions, programs, 0);
